...
3. **Choose and pin the base model.** Before training, check the base model on three counts: (1) size against your latency and cost budget, (2) coverage of every language your system serves, and (3) how much safety alignment it already carries. Prefer a model that already performs adequately on critical safety evaluations. Use fine-tuning only for specific, measurable residual gaps, and rerun the complete system evaluation afterwards.
