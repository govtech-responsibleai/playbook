...
| [SALAD-Bench](https://github.com/Libr-AI/do-not-answer) | ~21k samples from 6 risk categories. Approach of fine-tuning GPT-3.5-turbo on harmful QA-pairs to generate more unsafe data, plus prompt expansion with attack/defence modifications, is easily adapted. |
...
| [AdvBench / GCG](https://github.com/llm-attacks/llm-attacks) | 500 harmful behaviours with adversarial suffixes derived via gradient search. Suffixes tend to be gibberish and easily defended via perplexity guards, but the seed behaviours are useful. |
...
When using an LLM to score safety eval outputs, validate it against human reviewers. See [Evaluation methods](methods.mdx) for the general approach including the Alternative Annotator Test.

<mark className="new-since-v1">*Coming soon — extend with safety-specific validation patterns.*</mark>
